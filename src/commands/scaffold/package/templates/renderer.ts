...
export function rendererServiceTemplate(name: string, scope?: string): GeneratedTemplate {
  ...
  return codeTemplate(
    [
      ...
      statement(genTypeImport("./types", [`${Pascal}Options`], knitworkOptions)),
      "",
...
export function rendererPluginTemplate(name: string, scope?: string): GeneratedTemplate {
  ...
  return codeTemplate(
    [
      ...
      statement(genImport("./renderer-service", [`${Pascal}RendererService`], knitworkOptions)),
      statement(genTypeImport("./types", [`${Pascal}Options`], knitworkOptions)),
      "",
...
export function rendererComposablesTemplate(name: string, scope?: string): GeneratedTemplate {
  ...
  return codeTemplate(
    [
      ...
      " */",
      "",
      statement(genImport("@gwenjs/core", ["onCleanup"], knitworkOptions)),
      statement(genImport("@gwenjs/core/system", ["useService"], knitworkOptions)),
      statement(genTypeImport("./types", [`${Pascal}Service`], knitworkOptions)),
      statement(genImport("./augment", undefined, knitworkOptions)),
      "",
      "/**",
      ...
      " * Resources registered via onCleanup are cleaned up automatically.",
      " *",
      " * @example",
      " * ```ts",
      ...
      `  const service = useService('renderer:${name}')`,
      "",
      "  onCleanup(() => {",
...
export function rendererAugmentTemplate(name: string, scope?: string): GeneratedTemplate {
  ...
  return codeTemplate(
    [
      "/**",
      ` * Declaration merging — types useService('renderer:${name}').`,
...
export function rendererIndexTemplate(name: string, _scope?: string): GeneratedTemplate {
  ...
  return codeTemplate(
    [
      ...
      statement(genExport("./plugin", [`${Pascal}Plugin`], knitworkOptions)),
      "",
      `// Composables — use${Pascal}() for runtime access inside defineActor()`,
      statement(genExport("./composables", [`use${Pascal}`], knitworkOptions)),
      "",
      "// Public types",
      `export type { ${Pascal}Options, ${Pascal}Service } from './types'`,
      "",
...
export function conformanceTestTemplate(name: string, scope?: string): GeneratedTemplate {
  ...
  return codeTemplate(
    [
      ...
      statement(
        genImport("../src/renderer-service", [`${Pascal}RendererService`], knitworkOptions),
      ),
      "",
