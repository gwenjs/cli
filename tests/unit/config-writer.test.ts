/**
 * Unit tests for `appendModuleToConfig` (config-writer utility)
 *
 * Tests the actual file-writing logic — no mocks on config-writer itself.
 */
...
import { appendModuleToConfig } from "../../src/utils/config-writer.js";

/** Creates an isolated temp directory for each test. */
async function makeTempDir(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "gwen-configwriter-test-"));
}
